...
export const getDateTimeConfig = (
    ...
    const disabledDate = momentDate => {
        return (
            momentDate &&
            !findConfig(momentDate) &&
            momentDate <= moment(beginDate)
        );
    };

    const disabledHours = () =>
        availableHours.filter(
            hour => hour < beginHour + duration || hour > endHour,
        );

    const disabledMinutes = hour => {
        if (hour > beginHour + duration && hour < endHour) {
            return [];
        } else if (hour === beginHour + duration && hour === endHour) {
            return availableMinutes.filter(
                minute => minute < beginMinute || minute > endMinute,
            );
        } else if (hour === beginHour + duration) {
