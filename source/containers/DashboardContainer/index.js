...
class DashboardContainer extends Component {
    ...
    _renderDashboardColumns = () => {
        ...
        return [ ...Array(dashboard.columns).keys() ].map((_, index) => {
            ...
            if (mode === 'employees') {
                const employeeLoad = load.find(
                    elem => elem.employeeId == employees[ index ].id,
                );
                employeeLoadCoefficient = employeeLoad
                    ? employeeLoad.loadCoefficient
                    : 0;
            }
                            ...
                            { result.map((order, index) =>
                                order.empty ? (
                                    ...
                                    <DashboardOrder
                                        ...
                                        status={ result[ index ].options.status }
                                        dashboardRef={ this._dashboardRef }
